    ...
    <section className="relative overflow-hidden bg-[#f7f2e8]">
      ...
      <div className="mx-auto grid max-w-6xl gap-12 px-5 pb-20 pt-20 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:px-6 lg:pb-28 lg:pt-28">
        <div>
          ...
          <h1 className="max-w-3xl text-balance text-5xl font-black leading-[.95] tracking-[-.05em] text-slate-950 sm:text-6xl lg:text-7xl">Lo que necesitás, <span className="bg-gradient-to-r from-orange-500 to-fuchsia-600 bg-clip-text text-transparent">cerca tuyo.</span></h1>
          <p className="mt-6 max-w-xl text-pretty text-lg leading-relaxed text-slate-650">Buscá o publicá lo que necesitás y conectate con servicios, comercios y emprendimientos de tu zona.</p>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Link href="/dashboard" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-slate-950 px-6 py-3 text-sm font-bold text-white transition hover:-translate-y-0.5">Entrar a VEZI <ArrowRight className="h-4 w-4" /></Link>
            <Link href="/dashboard/necesito/nueva" className="inline-flex min-h-12 items-center justify-center rounded-2xl border-2 border-slate-950 px-6 py-3 text-sm font-bold transition hover:bg-white">Publicar una necesidad</Link>
          </div>
        </div>
        ...
        <div className="relative" aria-label="Vista previa de una búsqueda en VEZI">
          ...
          <div className="relative rounded-[2rem] border border-slate-800 bg-slate-950 p-4 shadow-2xl sm:p-6">
            <div className="flex items-center gap-3 rounded-2xl bg-white p-4 text-sm font-medium text-slate-700"><Search className="h-5 w-5 text-orange-500" /> Necesito profesora de matemática</div>
            <p className="mb-3 mt-6 text-xs font-bold uppercase tracking-widest text-orange-300">Resultados cerca tuyo</p>
            <div className="space-y-3">
              <div className="rounded-2xl bg-sky-500 p-4 text-white"><div className="flex items-center gap-2 text-xs font-bold uppercase"><Wrench className="h-4 w-4" /> Servicio · a 1,2 km</div><p className="mt-2 font-bold">Laura · Profesora de matemática</p><p className="mt-1 text-xs text-white/80">Clases de apoyo primario y secundario</p></div>
