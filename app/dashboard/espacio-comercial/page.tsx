...
function CommercialPreviewCard({ title, description, badge, meta, href, type }: { title: string; description: string; badge: string; meta: string; href: string; type: "commerce" | "entrepreneur" }) {
  ...
  return (
    <Link href={href} className={`group rounded-[24px] border p-5 transition-all hover:-translate-y-0.5 hover:shadow-sm ${isCommerce ? "border-sky-200 bg-sky-50" : "border-violet-200 bg-violet-50"}`}>
      <div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${isCommerce ? "bg-sky-100 text-sky-700" : "bg-violet-100 text-violet-700"}`}>{isCommerce ? <Store className="h-5 w-5" /> : <Sparkles className="h-5 w-5" />}</div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {showMyBusiness ? (
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button asChild className="bg-violet-600 text-white hover:bg-violet-700"><Link href="/dashboard/services"><Search className="mr-2 h-4 w-4" />Ofrecer servicio</Link></Button>
            </div>
          )}
        </div>
      </div>
