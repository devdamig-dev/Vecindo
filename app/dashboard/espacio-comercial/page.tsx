...
function CommercialPreviewCard({ title, description, badge, meta, href, type }: { title: string; description: string; badge: string; meta: string; href: string; type: "commerce" | "entrepreneur" }) {
  ...
  return (
    <Link href={href} className={`group rounded-[24px] border p-5 transition-all hover:-translate-y-0.5 hover:shadow-sm ${isCommerce ? "border-stone-200 bg-white" : "border-stone-200 bg-white"}`}>
      <div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${isCommerce ? "bg-[#e9efe6] text-primary" : "bg-[#e9efe6] text-primary"}`}>{isCommerce ? <Store className="h-5 w-5" /> : <Sparkles className="h-5 w-5" />}</div>
  ...
  return (
    <div className="flex max-w-full flex-col gap-6">
      <div className="space-y-1">
        <div className="inline-flex items-center gap-2 rounded-full bg-[#e9efe6] px-3 py-1 text-xs font-medium text-primary"><Store className="h-3.5 w-3.5" />Espacio comercial</div>
      ...
      <div className="rounded-[28px] border border-stone-200 bg-stone-950 p-5 text-white">
        ...
        <div className="rounded-[24px] border border-stone-200 bg-white p-5">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex items-start gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#e9efe6] text-primary"><Handshake className="h-5 w-5" /></div>
              <div>
            ...
            <Button asChild className="bg-primary text-primary-foreground hover:bg-primary/90"><Link href="/dashboard/suscripciones">{role === "service_provider" ? "Activar plan Red local" : "Activar acceso Comunidad"}</Link></Button>
          </div>
        </div>
      )}
      ...
      <Tabs defaultValue="comercios" className="w-full">
        ...
        <TabsContent value="comercios" className="mt-5 space-y-4">
          <div className="rounded-[24px] border border-stone-200 bg-white p-5">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                <div className="flex items-center gap-2 text-primary"><MapPin className="h-4 w-4" /><span className="font-medium">Comercios</span></div>
              ...
              <Button asChild className="bg-primary text-primary-foreground shadow-[0_6px_20px_rgba(0,0,0,0.08)] transition-all duration-200 hover:scale-[1.01] hover:bg-primary/90"><Link href="/dashboard/comercios">Explorar comercios</Link></Button>
            </div>
          </div>
        ...
        <TabsContent value="emprendimientos" className="mt-5 space-y-4">
          <div className="rounded-[24px] border border-stone-200 bg-white p-5">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                <div className="flex items-center gap-2 text-primary"><Sparkles className="h-4 w-4" /><span className="font-medium">Emprendimientos locales</span></div>
              ...
              <Button asChild className="bg-primary text-primary-foreground shadow-[0_6px_20px_rgba(0,0,0,0.08)] transition-all duration-200 hover:scale-[1.01] hover:bg-primary/90"><Link href="/dashboard/comercios?tipo=emprendimientos">Explorar emprendimientos</Link></Button>
            </div>
          </div>
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {showMyBusiness ? (
            ...
            <Button asChild className="bg-primary text-primary-foreground hover:bg-primary/90"><Link href="/dashboard/suscripciones"><Handshake className="mr-2 h-4 w-4" />Activar plan Red local</Link></Button>
          ) : (
            <div className="flex flex-wrap gap-2">
              ...
              <Button asChild className="bg-primary text-primary-foreground shadow-[0_6px_20px_rgba(0,0,0,0.08)] transition-all duration-200 hover:scale-[1.01] hover:bg-primary/90"><Link href="/dashboard/services/new"><Search className="mr-2 h-4 w-4" />Ofrecer servicio</Link></Button>
            </div>
          )}
        </div>
      </div>
